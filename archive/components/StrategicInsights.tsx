...
interface StrategicInsightsProps {
  ...
  isAnalyzing?: InsightId | null;
}

const StrategicInsights: React.FC<StrategicInsightsProps> = ({ data, onAnalyze, isIntegrationMode, isAnalyzing }) => {
  const { insight1, insight2, insight3 } = data;
  // Use the prop if provided, otherwise use local state
  const isAnalysisInProgress = isAnalyzing !== undefined ? isAnalyzing : null;
  ...
  const insight1Data = {
    datasets: insight1.initiatives.map((d, index) => ({
        label: `${d.name} (${d.id})`, // Use unique label to prevent React key conflicts
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Panel className="col-span-1">
            <div className="flex justify-between items-start mb-2">
                ...
                {!isIntegrationMode && <button 
                  onClick={() => onAnalyze('insight1')} 
                  className="text-sm bg-[var(--component-text-accent)] bg-opacity-80 hover:bg-opacity-100 px-3 py-1 rounded-md flex-shrink-0 ml-2"
                >
                  <span className="flex items-center">
                    {isAnalysisInProgress === 'insight1' ? (
                      <span className="animate-spin mr-1">↻</span>
                    ) : (
                      <span>✨</span>
                    )}
                    <span className="ml-1">{isAnalysisInProgress === 'insight1' ? 'Analyzing...' : 'Analyze'}</span>
                  </span>
                </button>}
            </div>
        ...
        <Panel className="col-span-1">
            <div className="flex justify-between items-start mb-2">
                ...
                {!isIntegrationMode && <button 
                  onClick={() => onAnalyze('insight2')} 
                  className="text-sm bg-[var(--component-text-accent)] bg-opacity-80 hover:bg-opacity-100 px-3 py-1 rounded-md flex-shrink-0 ml-2"
                >
                  <span className="flex items-center">
                    {isAnalysisInProgress === 'insight2' ? (
                      <span className="animate-spin mr-1">↻</span>
                    ) : (
                      <span>✨</span>
                    )}
                    <span className="ml-1">{isAnalysisInProgress === 'insight2' ? 'Analyzing...' : 'Analyze'}</span>
                  </span>
                </button>}
            </div>
        ...
        <Panel className="col-span-1">
             <div className="flex justify-between items-start mb-2">
                ...
                {!isIntegrationMode && <button 
                  onClick={() => onAnalyze('insight3')} 
                  className="text-sm bg-[var(--component-text-accent)] bg-opacity-80 hover:bg-opacity-100 px-3 py-1 rounded-md flex-shrink-0 ml-2"
                >
                  <span className="flex items-center">
                    {isAnalysisInProgress === 'insight3' ? (
                      <span className="animate-spin mr-1">↻</span>
                    ) : (
                      <span>✨</span>
                    )}
                    <span className="ml-1">{isAnalysisInProgress === 'insight3' ? 'Analyzing...' : 'Analyze'}</span>
                  </span>
                </button>}
            </div>
